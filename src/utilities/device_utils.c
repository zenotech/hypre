                          ...
                          hypre_ulonglongint offset)
{
   ...
   {
      /* WM: Question - Is creating the engines each time in this way inefficient? */
      /*                Is there any anologue to the cu/rocrand way of creating the generators? */
      std::uint64_t offset = idx.get_linear_id();
      oneapi::mkl::rng::device::philox4x32x10<> engine(1234ULL, offset);
      oneapi::mkl::rng::device::uniform<> distr(0., 1.);
