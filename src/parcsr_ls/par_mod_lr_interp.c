...
/*---------------------------------------------------------------------------
 * hypre_BoomerAMGBuildModSimpleExtPIInterp
 *  Comment:
 *--------------------------------------------------------------------------*/
HYPRE_Int
hypre_BoomerAMGBuildModSimpleExtPIInterp(hypre_ParCSRMatrix *A, HYPRE_Int *CF_marker,
                              hypre_ParCSRMatrix   *S, HYPRE_BigInt *num_cpts_global,
                              HYPRE_Int num_functions, HYPRE_Int *dof_func, // first version for num_functions = 1 only!
                              HYPRE_Int debug_flag,
                              HYPRE_Real strong_th,
                              ...
                              hypre_ParCSRMatrix  **P_ptr)
{
   ...
   /* Intermediate matrices */
   hypre_ParCSRMatrix *As_FF, *As_FC, *W;
   HYPRE_Real *D_beta, *D_gamma, *D_lambda, *D_tau, *D_tmp;
   hypre_CSRMatrix *As_FF_diag;
   hypre_CSRMatrix *As_FF_offd;
   hypre_CSRMatrix *As_FC_diag;
   hypre_CSRMatrix *As_FC_offd;
   ...
   D_gamma = hypre_CTAlloc(HYPRE_Real, n_Fpts, HYPRE_MEMORY_HOST);
...
#pragma omp barrier
      ...
      for (i=startf; i < stopf; i++)
      {
         ...
         if (count) 
            D_lambda[i] /= (HYPRE_Real)count;
         else
            D_lambda[i] = 0.0;
         D_tmp[i] = D_lambda[i]/(D_beta[i]+D_lambda[i]);
      }
      ...
      for (i=start; i < stop; i++)
      {
         if (CF_marker[i] < 0)
         {
            ...
            {
               D_gamma[row] += A_diag_data[j];
            }
            for (j=A_offd_i[i]; j < A_offd_i[i+1]; j++)
            {
               D_gamma[row] += A_offd_data[j];
            }
            for (j=As_FF_diag_i[row]; j < As_FF_diag_i[row+1]; j++)
            {
               D_gamma[row] -= As_FF_diag_data[j];
            }
            for (j=As_FF_offd_i[row]; j < As_FF_offd_i[row+1]; j++)
            {
               D_gamma[row] -= As_FF_offd_data[j];
            }
            D_gamma[row] -= D_beta[row];
            row++;
         }
      }
...
#pragma omp barrier
      ...
      {
         j = As_FF_diag_i[i];      
         beta = -1.0/(As_FF_diag_data[j]+D_gamma[i]+D_tau[i]); 
         As_FF_diag_data[j] = D_beta[i] + D_lambda[i];
         for (j=As_FF_diag_i[i]; j < As_FF_diag_i[i+1]; j++)
            As_FF_diag_data[j] *= beta;
         for (j=As_FF_offd_i[i]; j < As_FF_offd_i[i+1]; j++)
            As_FF_offd_data[j] *= beta;
         gamma = 1.0/(D_beta[i]+D_lambda[i]);
         for (j=As_FC_diag_i[i]; j < As_FC_diag_i[i+1]; j++)
            As_FC_diag_data[j] *= gamma;
         for (j=As_FC_offd_i[i]; j < As_FC_offd_i[i+1]; j++)
            As_FC_offd_data[j] *= gamma;
...
#pragma omp parallel for private(i) HYPRE_SMP_SCHEDULE
   ...
   *P_ptr = P;
   ...
   hypre_TFree(D_gamma, HYPRE_MEMORY_HOST);
