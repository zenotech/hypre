...
#pragma omp barrier
      ...
      if (my_thread_num == 0)
      {
         ...
         hypre_MPI_Allgather(&n_Fpts, 1, HYPRE_MPI_BIG_INT, &fpts_starts[1], 1, HYPRE_MPI_INT, comm);
...
#pragma omp barrier
      ...
      if (my_thread_num == 0)
      {
         ...
         hypre_MPI_Allgather(&n_Fpts, 1, HYPRE_MPI_BIG_INT, &fpts_starts[1], 1, HYPRE_MPI_INT, comm);
         hypre_MPI_Allgather(&n_new_Fpts, 1, HYPRE_MPI_BIG_INT, &fnew_pts_starts[1], 1, HYPRE_MPI_INT, comm);
         for (i = 2; i < num_procs+1; i++)
         {
            fpts_starts[i] += fpts_starts[i-1];
