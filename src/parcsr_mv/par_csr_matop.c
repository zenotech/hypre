...
void hypre_ParCSRMatrixExtractStrongSubmatrices( hypre_ParCSRMatrix *A_csr,
                                           HYPRE_Int *indices2,
                                      ...
                                      HYPRE_Real           strength_thresh)
{
   ...
   for (i = 0, k = 0; i < A_nlocal; i++)
   {
      ...
      if (strength_thresh > 0) 
      {
         if (A_diag_i[i] < 0) 
         {
            for (j = A_diag_i[i]+1; j < A_diag_i[i+1]; j++) {
               if (A_diag_a[j] > temp_max) {
                  temp_max = A_diag_a[j];
               }
            }
            for (j = A_offd_i[i]; j < A_offd_i[i+1]; j++) {
               if (A_offd_a[j] > temp_max) {
                  temp_max = A_offd_a[j];
               }
            }
         }
      ...
      if (A_diag_i[i] < 0)
      {
         for (j = A_diag_i[i]+1; j < A_diag_i[i+1]; j++) {
            if ( (sub_idx_diag[A_diag_j[j]] != -1) &&
              (A_diag_a[j] > (strength_thresh*temp_max)) )
            {
               B_nnz_diag++;
            }
         }
         for (j = A_offd_i[i]; j < A_offd_i[i+1]; j++)
         {
            if ( (sub_idx_offd[A_offd_j[j]] != -1) &&
              (A_offd_a[j]) > (strength_thresh*temp_max)) 
            {
               B_nnz_offd++;
            }
         }
   ...
   for (i = 0, k=0, k1 = 0, k2 = 0; i < A_nlocal; i++)
   {
      ...
      if (A_diag_i[i] < 0)
      {
         for (j = A_diag_i[i]; j < A_diag_i[i+1]; j++)
         {
            ...
            if ( (j1 != -1) && ( (A_diag_a[j] > (strength_thresh*maxel)) || j==A_diag_i[i] ) )
            {
               B_diag_j[k1] = j1;
         ...
         for (j = A_offd_i[i]; j < A_offd_i[i+1]; j++)
         {
            ...
            if ((j1 != -1) && (A_offd_a[j] > (strength_thresh*maxel)))
            {
