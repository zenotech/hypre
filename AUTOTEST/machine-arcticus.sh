...
# SYCL without UM [struct]
co="--with-sycl"
ro="-struct -rt -save ${save} -rtol ${rtol} -atol ${atol}"
./test.sh basic.sh $src_dir -co: $co -mo: $mo -ro: $ro
./renametest.sh basic $output_dir/basic-sycl-nonum

# SYCL with UM [ij, struct]
co="--with-sycl --enable-unified-memory"
ro="-ij-gpu -struct -rt -save ${save} -rtol ${rtol} -atol ${atol}"
./test.sh basic.sh $src_dir -co: $co -mo: $mo -ro: $ro
